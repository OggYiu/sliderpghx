...
class Kernal extends Process {
	public var curScene( default, null ) : Scene = null;
	// public var curCamera( default, default ) : Camera = null;
...
//		var startSceneId : String = SceneGame.ID;
	...
	function refreshCurrentScene() : Void {
		Helper.assert( curScene != null, "current scene is null" );
		changeScene( curScene.id );
	}
	...
	override private function update_(dt:Float):Void 
	{
        ...
        CommandManager.getInstance().update( dt );
	}
