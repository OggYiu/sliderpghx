 ...
 */
 ...
 class MSpriteLoader {
	...
	public function new() {
 	...
 	public function loadMSprite( dataPath:String, imgPath:String, splitImageClips:Bool, imageloader:MSpriteImageLoader ) : MSpriteData {
		...
		var targetFileName : String = dataPath + ".anu";
			...
			for( i in 0 ... noOfImages ) {
				...
				for( j in 0 ... noOfClipsInThisImage ) {
					...
					if(splitImageClips){
						...
						imageArrayForClips[j] = imageloader.loadImageClip(imgPath,i,x,y,w,h,(orientationUsedInStudio>>1));
